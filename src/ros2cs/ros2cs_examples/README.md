...
Before building or running examples, source your ROS 2 Jazzy environment in the
current terminal. On Ubuntu this is usually `source /opt/ros/jazzy/setup.bash`;
on Windows, call the ROS 2 setup batch file for your installation.
...
After receiving the desired number of samples, listener will print average latency and a
`PERFORMANCE_LISTENER_COMPLETE ...` summary line.

This probe is a manual smoke/performance aid, not a release-grade benchmark.
Latency is computed from timestamps generated by independent clocks in the talker and
listener, so the result is meaningful only when both processes run on the same machine
or on machines with synchronized system clocks. `SENSOR_DATA` uses BEST_EFFORT
reliability and may drop messages under load. The `Benchmark("Publish")` block in the
talker measures the publish call at debug log level; use external tools such as
`ros2 topic hz /perf_chatter` when you need an explicit throughput number.

### Example results

The table below is from the original example environment and should be treated as a
historical reference. Re-benchmark on your current hardware and runtime before using
